...
namespace WebGoatCore.Utils
{
    ...
    public class Argon2Hasher<TUser>: IPasswordHasher<TUser> where TUser : class
    {
        ...
        private static byte[] HashPassword(string password)
        {
            ...
            // 3. Using the password and salt, generate an Argon2 hash
            byte[] hash = GenerateArgon2Hash(passwordBytes, salt);
        ...
        private static bool VerifyHashedPassword(string hashedPassword, string providedPassword)
        {
            try
            {
                ...
                // 2. Extract the salt from the hashed password bytes
                byte[] salt = new byte[saltSize];
                Buffer.BlockCopy(hashedPasswordBytes, 0, salt, 0, saltSize);

                // 3. Extract the actual password hash from hashed password bytes
                byte[] expectedSubkey = new byte[hashSize];
                Buffer.BlockCopy(hashedPasswordBytes, saltSize, expectedSubkey, 0, hashSize);

                // 4. Using the Argon 2 hashing algorithm, hash the providedPassword string
                byte[] actualSubkey = GenerateArgon2Hash(System.Text.Encoding.UTF8.GetBytes(providedPassword), salt);

                // 5. Compare the "expectedSubkey" and "actualSubkey" byte arrays securely
